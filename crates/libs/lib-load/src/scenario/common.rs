...
pub async fn create_order(user_ctx: &mut UserContext, order_content: OrderContent) -> OrderId {
    user_ctx.post_rpc("create_order", json!(order_content)).await
}

pub async fn check_order(user_ctx: &mut UserContext, order_id: OrderId) -> OrderStored {
